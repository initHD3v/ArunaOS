...
describe('SecurityRatingSystem', () => {
  ...
  describe('scoreToLevel', () => {
    it('should return trusted for score >= 80', () => {
      const result = analyzer.analyze(baseManifest({
        permissions: [],
        signature: 'signed',
        author: 'Author',
    ...
    it('should return high-risk for score < 40', () => {
      const result = analyzer.analyze(baseManifest({
        version: '0.0.1',
        permissions: ['camera', 'microphone', 'geolocation', 'storage:read', 'storage:write', 'network'],
      }));
  ...
  describe('permission analysis', () => {
    it('should give 30 for no permissions', () => {
      const result = analyzer.analyze(baseManifest({ permissions: [] }));
      const perm = result.breakdown.find((b) => b.category === 'Permissions');
      expect(perm?.score).toBe(30);
      expect(perm?.maxScore).toBe(30);
    });

    it('should deduct for sensitive permissions', () => {
      const result = analyzer.analyze(baseManifest({ permissions: ['camera'] }));
      const perm = result.breakdown.find((b) => b.category === 'Permissions');
      expect(perm!.score).toBeLessThan(30);
      expect(result.warnings.some((w) => w.includes('camera'))).toBe(true);
    });

    it('should warn for many permissions', () => {
      const result = analyzer.analyze(baseManifest({
        permissions: ['storage:read', 'storage:write', 'network', 'notification', 'clipboard:write'],
      }));
  ...
  describe('checksum analysis', () => {
    it('should give 20 points when verified', () => {
      const result = analyzer.analyze(baseManifest(), { checksumVerified: true });
      const cs = result.breakdown.find((b) => b.category === 'Checksum');
      expect(cs?.score).toBe(20);
    });

    it('should give 0 and warn when not verified', () => {
      const result = analyzer.analyze(baseManifest());
      const cs = result.breakdown.find((b) => b.category === 'Checksum');
  ...
  describe('source analysis', () => {
    it('should give 15 points for registry', () => {
      const result = analyzer.analyze(baseManifest(), { source: 'registry' });
      const src = result.breakdown.find((b) => b.category === 'Source');
      expect(src?.score).toBe(15);
    });

    it('should give 5 points for URL', () => {
      const result = analyzer.analyze(baseManifest(), { source: 'url' });
      const src = result.breakdown.find((b) => b.category === 'Source');
  ...
  describe('metadata analysis', () => {
    it('should give full points with complete metadata', () => {
      const result = analyzer.analyze(baseManifest({
        author: 'Dev',
        homepage: 'https://example.com',
    ...
    it('should give partial points for incomplete metadata', () => {
      const result = analyzer.analyze(baseManifest({}));
      const meta = result.breakdown.find((b) => b.category === 'Metadata');
      expect(meta!.score).toBeLessThan(15);
    });

    it('should warn when metadata is incomplete', () => {
      const result = analyzer.analyze(baseManifest({}));
  ...
  describe('version analysis', () => {
    it('should give low score for v0.x', () => {
      const result = analyzer.analyze(baseManifest({ version: '0.1.0' }));
      const ver = result.breakdown.find((b) => b.category === 'Version');
      expect(ver!.score).toBeLessThanOrEqual(5);
    });

    it('should give higher score for stable versions', () => {
      const result2 = analyzer.analyze(baseManifest({ version: '2.0.0' }));
  ...
  describe('signature analysis', () => {
    it('should give 10 points when signature present', () => {
      const result = analyzer.analyze(baseManifest({ signature: 'abc123' }));
      const sig = result.breakdown.find((b) => b.category === 'Signature');
      expect(sig?.score).toBe(10);
    });

    it('should give 0 when no signature', () => {
      const result = analyzer.analyze(baseManifest());
      const sig = result.breakdown.find((b) => b.category === 'Signature');
  ...
  describe('warnings', () => {
    it('should return multiple warnings for risky module', () => {
      const result = analyzer.analyze(baseManifest({
        version: '0.0.1',
